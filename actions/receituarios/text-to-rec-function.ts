...
export default async function textToRecFunction(text: string, receita?: string): Promise<{ success: boolean, output?: string, message?: string }> {
    ...
    const array = text
        ...
        .map((string) => {
            ...
            let splited = string.split(" ").filter(elem => elem != "");
            ...
            const { posologiaText, quantidade } = posologiaQntText(isSN, isTempo, isModificador, splited[posologiaIndex], modPos, tempo)
   ...
   let string = "Uso Oral:\n\n"
    ...
    if (!receita) {
       finalArray.forEach((element, index) => {
            string = string + (index + 1) + ")" + element.med + " " + element.dosagem + " " + element.hyphen + " " + element.quantidade + "\n" + element.posologiaText + ".\n" + "\n"
...
///funçoes essenciais
function posologiaQntText(isSN: boolean, isTempo: boolean, isModificador: boolean, posologia: string, modPos: string, tempo: string): { isError: boolean, posologiaText: string, quantidade: string } {
    ...
    const mid = { regEx: new RegExp("(MID|24\/24.*|1x|qd|sid)", "gi"), statement: `Tomar ${cps} ${unidade}${plural} ao dia`, quantidade: (1 * multiplicador * cps), unitario: 1 };
    const bid = { regEx: new RegExp("(BID|12\/12.*|2x)", "gi"), statement: `Tomar ${cps} ${unidade}${plural} de 12/12 horas`, quantidade: (2 * multiplicador * cps), unitario: 2 };
    const tid = { regEx: new RegExp("(TID|8\/8.*|3x)", "gi"), statement: `Tomar ${cps} ${unidade}${plural} de 8/8 horas`, quantidade: (3 * multiplicador * cps), unitario: 3 };
    const qid = { regEx: new RegExp("(QID|6\/6.*|4x)", "gi"), statement: `Tomar ${cps} ${unidade}${plural} de 6/6 horas`, quantidade: (4 * multiplicador * cps), unitario: 4 };
    ...
    if (mid.regEx.test(posologia)) { finalStatement = mid.statement; result = mid.quantidade; unitario = mid.unitario }
        ...
        let statement = `Tomar ${cps} ${unidade}${plural} de ${qNumber}/${qNumber} horas `
...
function isTempoModifier(cps: number, tempo: string, unitario: number) {
    let result = unitario
    let num = parseInt(tempo.replace(/[^0-9]/g, ""))
    let dma = tempo.replace(/[^a-z]/g, "")
    ...
    let total = `${(num * mult * result * cps)} comprimidos`
