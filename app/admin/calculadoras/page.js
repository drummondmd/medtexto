        ...
        async function fetchData() {
            try {
                ...
                const parsed = res.map(c => ({
                    ...c,
                    _id: c._id?.toString(),
                    calculadorasRelacionadas: c.calculadorasRelacionadas?.map(rel => ({
                        ...rel,
    ...
    return (
        <div>
            <div className="d-flex justify-content-between align-items-center mb-4">
                <h2>Calculadoras</h2>
                <Link href="/admin/calculadoras/nova" className="btn btn-primary">+ Nova Calculadora</Link>
            </div>
                    ...
                    <tbody>
                        {calculadoras.map((calc) => (
                            <tr key={JSON.stringify(calc._id)}>
                                ...
                                <td>{calc.slug}</td>
                                <td>
                                    <Link href={`/admin/calculadoras/editar/${calc.slug}`} className="btn btn-sm btn-warning me-2">Editar</Link>
                                    <button className="btn btn-sm btn-danger" onClick={() => handleDelete(calc.slug)}>Excluir</button>
                                </td>
                            </tr>
                        ))}
                    </tbody>
