...
export default function CreateNota({
  ...
  const emptyArrayContent = isCaderno ? (
    <div className="text-sm text-gray-600">
      Nenhum caderno encontrado até o momento, adicione o primeiro{newCadernoInput}
    </div>
  ) : null;
  ...
  return (
    <div
      className="fixed top-0 right-0 bottom-0 left-0 bg-black/70 flex items-center justify-center z-50"
      onClick={close}
    >
      <div
      ...
      >
        <form action={formAction}>
          <div className="flex justify-end mb-3">
            <button
              type="button"
              className="text-sm px-3 py-1 rounded-lg border border-solid	 border-gray-800 hover:bg-gray-400"
              onClick={close}
            >
              Fechar
            </button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-3">
              <div>
                <label className="block text-sm font-medium text-gray-700">
                  Adicionar caderno existente:
                </label>
                <select
                  name="cadernoExistente"
                  className="mt-1 block w-full rounded-md border-gray-300 shadow-sm py-2 px-3 focus:outline-none focus:ring-2 focus:ring-blue-500"
                >
          ...
          <input name="userId" value={user.user_id || userId} readOnly hidden />
