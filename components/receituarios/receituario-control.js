            ...
            <div className="row">
                ...
                <div className="col-md-10">
                    {cadernos.map((caderno) => {
                        return (
                            <button type="button" key={caderno["_id"]} onClick={(e) => clickHandler(e)} className={`btn m-1 ${caderno.titulo===selected.titulo?"btn-secondary":"btn-outline-secondary"}`} id={caderno["_id"]}>{caderno.titulo}</button>
                        )
                    }
                    )}
