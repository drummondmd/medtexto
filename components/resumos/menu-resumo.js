...
export default function MenuResumo({ cadernosJson, userNameSlug, userDetail }) {
  ...
  let isContent = false;
  const cadernos = JSON.parse(cadernosJson);
        ...
        {isContent ? (
          <>
            <li className=" p-3 list-group-item">
              <Link
                className="link-dark link-underline-opacity-25 link-underline-opacity-75-hover"
                href={userNameHref}
              >
                Todas as notas
              </Link>
