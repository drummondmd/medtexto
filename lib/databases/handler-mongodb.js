...
async function updateReceituarioTransforma(user, conteudo) {
  ...
  ///fazer validação de erros depois
  try {
    await UsuarioMongo.updateOne({ user_id: user.id }, { $set: { "receituarioTransforma.input": conteudo.input } })
    await UsuarioMongo.updateOne({ user_id: user.id }, { $set: { "receituarioTransforma.output": conteudo.output } })
  } catch (error) {
    console.error(error, "Erro ao atualizar dados no mongoDB")
  }
