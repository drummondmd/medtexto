...
async function createUserMongo(user) {
  try {
    ...
    console.error(error, "Erro ao inserir entrada na mongoDB");
...
const getUserMongo = unstable_cache(
  async function getUserMongo(user_id) {
    await connectToDatabase();
    try {
      const result = await UsuarioMongo.findOne({ user_id: user_id }).lean();
      if (result.length === 0) {
        console.error("Resultado vazio,usuario não encontrado");
        revalidateTag("user-mongo", "max");
        return null;
      }
      return result;
    } catch (error) {
      console.error(error, "Erro ao buscar dados na MongoDb");
      revalidateTag("user-mongo", "max");
      return null;
    }
  },
  ...
  async function createNote() {
    try {
      const result = await UsuarioMongo.updateOne(
        { user_id: userId },
        { $push: { [resource]: { titulo: data.titulo, conteudo: data.conteudo } } }
      );
      if (result.modifiedCount > 0) {
        response = true;
      } else {
        console.error("Algum erro aconteceu ao inserir dado dentro do array");
        response = false;
      }
    } catch (error) {
  ...
  async function deleteNote() {
    try {
      ...
      if (result.modifiedCount > 0) {
        ...
        console.error("Não encontrado arquivo solicitado para alterar");
        response = false;
      }
    } catch (error) {
      ...
      const result = await UsuarioMongo.updateOne(
        { user_id: userId, [`${resource}._id`]: noteId },
        {
          $set: {
            ...
            [`${resource}.$.conteudo`]: data.conteudo,
          },
        }
      );
      if (result.modifiedCount > 0) {
...
async function handleyArrayofArrayOfNotes(resource, crud, userId, data, isNewArray, caderno_id) {
  ...
  async function createCaderno() {
    ...
    try {
      const result = await UsuarioMongo.updateOne(
        { user_id: userId },
        { $push: { [resource]: newObj } }
      );
      if (result.modifiedCount > 0) {
        return newObj["_id"];
      } else {
        console.error("Algum erro aconteceu criar caderno");
        return null;
      }
    } catch (error) {
      console.error("erro ao inserir dados no mongo", error);
  ...
  async function deleteCaderno() {
    try {
      ...
      if (result.modifiedCount > 0) {
        ...
        console.error("Algum erro aconteceu ao apagar dado dentro do array");
        response = false;
      }
    } catch (error) {
      console.error("Erro ao deletar nota na Mongo", error);
  ...
  async function updateNote() {
    try {
      ...
      if (result.modifiedCount > 0) {
        ...
        console.error("Algum erro aconteceu ao atualizar dado dentro do array", result);
        response = false;
      }
    } catch (error) {
...
async function updateBloco(user, conteudo) {
  ...
  try {
    await UsuarioMongo.updateOne({ user_id: user.user_id }, { $set: { blocosDeNotas: conteudo } });
  } catch (error) {
...
async function updateReceituarioTransforma(user, conteudo) {
  ...
  try {
    await UsuarioMongo.updateOne(
      { user_id: user.id },
      { $set: { "receituarioTransforma.input": conteudo.input } }
    );
    await UsuarioMongo.updateOne(
      { user_id: user.id },
      { $set: { "receituarioTransforma.output": conteudo.output } }
    );
  } catch (error) {
    console.error(error, "Erro ao atualizar dados no mongoDB");
  }
...
const getCalculadoras = unstable_cache(
  async function getCalculadoras() {
    await connectToDatabase();
    // await new Promise((resolve) => setTimeout(resolve, 2000));
    try {
      const result = await Calculadora.find(
        {},
        { titulo: 1, descricao: 1, slug: 1, _id: 1 }
      ).lean();

      if (!result || result.length === 0) {
        console.error("Nenhuma Calculadora encontrada");
        return null;
      }
...
const getCalculadora = unstable_cache(
...
export {
  ...
  getUserMongo,
  updateBloco,
  updateReceituarioTransforma,
  handleArrayOfNotes,
  handleyArrayofArrayOfNotes,
  getCalculadoras,
  getCalculadora,
};
