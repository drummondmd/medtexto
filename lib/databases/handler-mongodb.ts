...
import { UsuarioMongo } from "./mongo-models";
...
const getUserMongo = unstable_cache(
  async function getUserMongo(user_id) {
    await connectToDatabase();
    try {
      const result = await UsuarioMongo.findOne({ user_id: user_id }).lean().exec();
...
export type NoteData = { _id: string; titulo: string; conteudo: string };

async function handleArrayOfNotes(
