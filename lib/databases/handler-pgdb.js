...
const getUser = async function getUser(userName, email) {
  let usuario;
...
async function updateUltimoLogin(user) {
  ...
  try {
    ...
    console.error("Erro na database ao incluir pessoa", error);
  }
}
...
async function loginRegistrer(username, id, cidade, estado) {
  ...
  try {
    ...
    console.error(error, "Erro ao inserar dado na database de login");
  }
}
...
export { getUser, createUser, updateUser, updateUltimoLogin, loginRegistrer, upadteHomepage };
