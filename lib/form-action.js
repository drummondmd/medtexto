...
} from "./databases/handler-mongodb";
import { getUser, updateUser, getUserProfile } from "./databases/handler-pgdb";
...
async function createUserForm(prevState, formData) {
  const user = {
    ...
    senhaConfirma: formData.get("senhaConfirma"),
  };
  /// checar se client ou user ja cadastrado

  const alreadyUser = await getUser(user.username, user.email);
  ...
  if (isInvalid(user.senha) || user.senha != user.senhaConfirma || user.senhaDesejada.length < 6) {
    return {
      ///if error
      payload: formData,
...
async function updateUserForm(prevState, formData) {
  ...
  const data = {
    ...
    id: prevInfo.user_id,
    data_nascimento: isInvalid(formData.get("data_nascimento"))
      ? prevInfo.data_nascimento
      : formData.get("data_nascimento"),
    tipo_usuario: isInvalid(formData.get("tipo_usuario"))
      ? prevInfo.tipo_usuario
      : formData.get("tipo_usuario"),
    instituicao_ensino: isInvalid(formData.get("instituicao_ensino"))
      ? prevInfo.instituicao_ensino
      : formData.get("instituicao_ensino"),
    ano_formacao: isInvalid(formData.get("ano_formacao"))
      ? prevInfo.ano_formacao
      : formData.get("ano_formacao"),
    especialidade: isInvalid(formData.get("especialidade"))
      ? prevInfo.especialidade
    ...
    local_trabalho: isInvalid(formData.get("local_trabalho"))
      ? prevInfo.local_trabalho
      : formData.get("local_trabalho"),
  };
...
async function createReceituario(prevState, formData) {
  ...
  if (isInvalid(data.userId) || isInvalid(data.titulo) || isInvalid(data.conteudo)) {
    return {
      payload: formData,
...
async function createResumoForm(prevState, formData) {
  ...
  if (
    data.cadernoExistente === null ||
    data.cadernoExistente === "" ||
    data.cadernoExistente == "Selecionar caderno..."
  ) {
    isNew = true;
  ...
  try {
    ...
    if (isNew) {
      ...
      result = await handleyArrayofArrayOfNotes(
        "resumos",
        "create",
        data.userId,
        data,
